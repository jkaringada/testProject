...
public class SvHealthCheckJdbcRepository implements HealthCheckServiceInterface {
	...
	@Override
	public SvHealthCheck findByClient(String clientname) {
	    return jdbcTemplate.queryForObject("select * from svhealthcheck where svclientname=? ORDER BY svid ASC ", new Object[] {
	    		clientname
	        },
	        new BeanPropertyRowMapper < SvHealthCheck > (SvHealthCheck.class));
	}
	...
	public List < SvHealthCheck > findAll() {
	    return jdbcTemplate.query("select * from svhealthcheck ORDER BY svid ASC ", new SvHealthCheckRowMapper());
	}
